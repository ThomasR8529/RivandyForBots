...
namespace ProceduralWorlds.SceneOptimizer
{
    ...
    public class SceneOptimizerEditor : SceneOptimizerBaseEditor
    {
        ...
        public static Color ActionButtonColor =>
            EditorGUIUtility.isProSkin ? ACTION_BUTTON_PRO_COLOR : ACTION_BUTTON_COLOR;
        ...
        public override void OnInspectorGUI()
        {
            ...
            scrollPos = EditorGUILayout.BeginScrollView(scrollPos, Styles.panel);
            {
                ...
                m_sceneOptimizationPanel = m_editorUtils.Panel("SceneOptimization", SceneOptimizationPanel,
                    m_sceneOptimizationPanel);
        ...
        private void SettingsPanel(bool helpEnabled)
        {
            Settings settings = m_sceneOptimizer.Settings;
            settings.SnapMode =
                (Constants.SnapMode)m_editorUtils.EnumPopup("SettingsSnapMode", settings.SnapMode, helpEnabled);
            settings.OffsetCheck = m_editorUtils.FloatField("SettingsOffsetCheck", settings.OffsetCheck, helpEnabled);
            settings.DistanceCheck =
                m_editorUtils.FloatField("SettingsDistanceCheck", settings.DistanceCheck, helpEnabled);
            settings.RaiseAndLowerAmount = m_editorUtils.FloatField("SettingsRaiseAndLowerAmount",
                settings.RaiseAndLowerAmount, helpEnabled);
        }
        ...
        private void KeyBindingsPanel(bool helpEnabled)
        {
            KeyBindings keyBindings = m_sceneOptimizer.KeyBindings;
            GUI.enabled = false;
            SceneOptimizer.m_firstKey =
                (KeyCode)m_editorUtils.EnumPopup("KeyBindingsHoldDownKey", SceneOptimizer.m_firstKey, helpEnabled);
            GUI.enabled = true;
            keyBindings.SnapToGroundKey = (KeyCode)m_editorUtils.EnumPopup("KeyBindingsSnapToGroundKey",
                keyBindings.SnapToGroundKey, helpEnabled);
            keyBindings.AlignToGroundKey = (KeyCode)m_editorUtils.EnumPopup("KeyBindingsAlignToSlopeKey",
                keyBindings.AlignToGroundKey, helpEnabled);
            keyBindings.AlignAndSnapToGroundKey = (KeyCode)m_editorUtils.EnumPopup("KeyBindingsSnapAndAlignToGroundKey",
                keyBindings.AlignAndSnapToGroundKey, helpEnabled);
            keyBindings.RaiseFromGroundKey = (KeyCode)m_editorUtils.EnumPopup("KeyBindingsRaiseFromGroundKey",
                keyBindings.RaiseFromGroundKey, helpEnabled);
            keyBindings.LowerInGroundKey = (KeyCode)m_editorUtils.EnumPopup("KeyBindingsLowerInGroundKey",
                keyBindings.LowerInGroundKey, helpEnabled);
            keyBindings.OptimizeKey =
                (KeyCode)m_editorUtils.EnumPopup("KeyBindingsOptimizeKey", keyBindings.OptimizeKey, helpEnabled);
        }
        ...
        private void OptimizeCommandPanel(OptimizeCommand command, bool helpEnabled)
        {
            ...
            command.ObjectFilterMode =
                (Constants.ObjectFilterMode)m_editorUtils.EnumPopup("ObjectFilterMode", command.ObjectFilterMode,
                    helpEnabled);
            command.SourceMeshLayers =
                m_editorUtils.LayerMaskField("SourceMeshLayers", command.SourceMeshLayers, helpEnabled);
            ...
            else
            {
                ...
                m_editorUtils.MinMaxSliderWithFields("ObjectSizeRange", ref minObjectSize, ref maxObjectSize, 0, 1024,
                    helpEnabled);
                if (Math.Abs(command.MaxObjectSize - minObjectSize) > 0.01f ||
                    Math.Abs(command.MaxObjectSize - maxObjectSize) > 0.01f)
                {
            ...
            else
            {
                ...
                EditorGUI.BeginChangeCheck();
                {
                    cellSize.x = cellSize.y =
                        cellSize.z = m_editorUtils.FloatField("CellSize", cellSize.x, helpEnabled);
            ...
            EditorGUI.indentLevel++;
            {
                ...
                command.MeshFormat =
                    (IndexFormat)m_editorUtils.EnumPopup("MeshFormat", command.MeshFormat, helpEnabled);
                bool performanceIssues = command.MeshFormat == IndexFormat.UInt32 && !IsStandalonePlatform();
                if (performanceIssues)
                {
                    EditorGUILayout.HelpBox(
                        "UInt32 can cause performance issues in the current platform. Consider switching to UInt16.",
                        MessageType.Warning);
                    m_warningsPresent |= performanceIssues;
                }

                command.VisualizationColor =
                    m_editorUtils.ColorField("VisualizationColor", command.VisualizationColor, helpEnabled);
                command.DisableRenderers =
                    m_editorUtils.Toggle("DisableRenderers", command.DisableRenderers, helpEnabled);
                bool backupNotification = command.DisableRenderers;
                if (backupNotification)
                {
                    EditorGUILayout.HelpBox(
                        "This setting will modify the original objects. You may want to make a backup of the original objects before combining.",
                        MessageType.Info);
            ...
            EditorGUI.indentLevel++;
            {
                ...
                if (command.AddLayerCulling)
                {
                    EditorGUI.BeginChangeCheck();
                    {
                        EditorGUI.indentLevel++;
                        {
                            command.TargetMeshLayer = m_editorUtils.LayerField("TargetMeshLayer",
                                command.TargetMeshLayer, helpEnabled);
                            GUI.SetNextControlName(OBJECT_LAYER_CULLING);
                            command.ObjectlayerCullingDistance = m_editorUtils.FloatField("ObjectDistance",
                                command.ObjectlayerCullingDistance, helpEnabled);
                            GUI.SetNextControlName(SHADOW_LAYER_CULLING);
                            command.ShadowLayerCullingDistance = m_editorUtils.FloatField("ShadowDistance",
                                command.ShadowLayerCullingDistance, helpEnabled);
                            command.ObjectVisualizationColor = m_editorUtils.ColorField("ObjectVizColor",
                                command.ObjectVisualizationColor, helpEnabled);
                            command.ShadowVisualizationColor = m_editorUtils.ColorField("ShadowVizColor",
                                command.ShadowVisualizationColor, helpEnabled);
                        }
                        EditorGUI.indentLevel--;
                    }
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        {
                            cullingSystem.SetObjectCullingDistance(command.TargetMeshLayer,
                                command.ObjectlayerCullingDistance);
                            cullingSystem.SetShadowCullingDistance(command.TargetMeshLayer,
                                command.ShadowLayerCullingDistance);
                        }
                    }
                }
            }
            ...
            EditorGUI.indentLevel++;
            {
                ...
                if (command.AddColliders)
                {
                    EditorGUI.indentLevel++;
                    {
                        command.AddColliderLayer =
                            m_editorUtils.LayerField("AddColliderLayer", command.AddColliderLayer);
                    }
                    EditorGUI.indentLevel--;
                }
            }
            ...
            EditorGUI.indentLevel++;
            if (command.UseLargeRanges)
                command.LodSizeMultiplier =
                    m_editorUtils.FloatField("LodSizeMultiplier", command.LodSizeMultiplier, helpEnabled);
            else
                command.LodSizeMultiplier =
                    m_editorUtils.Slider("LodSizeMultiplier", command.LodSizeMultiplier, 0f, 1f, helpEnabled);
            command.AddLodGroup = m_editorUtils.Toggle("AddLodGroup", command.AddLodGroup, helpEnabled);
            if (command.AddLodGroup)
            {
                EditorGUI.indentLevel++;
                if (command.UseLargeRanges)
                    command.LodCullPercentage =
                        m_editorUtils.FloatField("LodCullPercentage", command.LodCullPercentage, helpEnabled);
                else
                    command.LodCullPercentage = m_editorUtils.Slider("LodCullPercentage", command.LodCullPercentage,
                        99f, 1f, helpEnabled);
                EditorGUI.indentLevel--;
            }
        ...
        private void SceneOptimizationPanel(bool helpEnabled)
        {
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            {
                ...
                if (tools.SaveToDisk)
                {
                    ...
                    {
                        EditorGUILayout.HelpBox(
                            "You have root entries that do not have a saved scene! These objects will be ignored until you save their respective scene.",
                            MessageType.Warning);
                ...
                {
                    EditorGUILayout.HelpBox(
                        "You need to provide Root GameObjects that contain Meshes in order to optimize.",
                        MessageType.Info);
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            {
                ...
                if (hasDuplicates)
                {
                    EditorGUILayout.HelpBox("You cannot have Optimize Commands with the same names!",
                        MessageType.Error);
                    m_errorsPresent |= hasDuplicates;
                }
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            {
                ...
                if (m_errorsPresent)
                {
                    EditorGUILayout.HelpBox("You need to fix all Errors above before Combining Meshes!",
                        MessageType.Error);
                }

                if (m_warningsPresent)
                {
                    EditorGUILayout.HelpBox("Be sure to address any warnings before Combining Meshes.",
                        MessageType.Warning);
                }
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (m_editorUtils.Button("ResetToDefaults", GUILayout.Height(30f)))
                    {
                        if (EditorUtility.DisplayDialog("Reset to Defaults",
                                "Are you sure you want to reset all of the Scene Optimization settings?", "Yes", "No"))
                    ...
                    {
                        EditorApplication.delayCall += () => { PerformSceneOptimization(); };
                    }
