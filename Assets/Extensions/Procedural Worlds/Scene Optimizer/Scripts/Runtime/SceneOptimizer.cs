    ...
    public class SceneOptimizer : SceneOptimizerInternal
    {
        ...
        public void PostProcessSceneOptimization(OptimizeCall optimizeCall, bool recordUndo = true)
        {
            ...
            if (optimizedRoots.Count > 0)
            {
                ...
                if (Tools.SaveToDisk)
                {
                    Dictionary<string, MeshFilter> meshAssets = new Dictionary<string, MeshFilter>();
                    Dictionary<string, MeshCollider> colliderAssets = new Dictionary<string, MeshCollider>();
                    ...
                    try
                    {
                        ...
                        foreach (GameObject rootObject in optimizedRoots)
                        {
                            ...
                            foreach (MeshFilter meshFilter in meshFilters)
                            {
                                ...
                                string path = PWEvents.SaveMeshToDisk(scene, mesh);

                                if (meshAssets.ContainsKey(path))
                                    continue;
                                meshAssets.Add(path, meshFilter);
                            }
                            ...
                            foreach (MeshCollider meshCollider in meshColliders)
                            {
                                ...
                                if (colliderAssets.ContainsKey(path))
                                    continue;
                                colliderAssets.Add(path, meshCollider);
                            }
                        }
                    }
                    finally
                    {
                        ...
                        foreach (KeyValuePair<string, MeshFilter> pair in meshAssets)
                        {
                            string meshPath = pair.Key;
                            MeshFilter meshFilter = pair.Value;
                            meshFilter.sharedMesh = AssetDatabase.LoadAssetAtPath<Mesh>(meshPath);
                        }

                        foreach (KeyValuePair<string, MeshCollider> pair in colliderAssets)
                        {
                            string meshPath = pair.Key;
                            MeshCollider meshCollider = pair.Value;
                            meshCollider.sharedMesh = AssetDatabase.LoadAssetAtPath<Mesh>(meshPath);
                        }
                    }
                }
                else
                ...
                foreach (KeyValuePair<SceneData, GameObject> pair in optimizeCall.remainingParents)
                {
                    SceneData sceneData = pair.Key;
                    if (sceneData == null)
                        continue;
                    GameObject parent = pair.Value;
                    if (parent == null)
                        continue;
