...
namespace Lightbug.CharacterControllerPro.Demo
{
    ...
    public class NormalMovement : CharacterState
    {
        ...
        public override void CheckExitTransition()
        {
            ...
            if (CharacterBrain.characterActions.jetPack.value)
            {
                CharacterStateController.EnqueueTransition<JetPack>();
            }
            else if (CharacterBrain.characterActions.dash.Started)
            {
            ...
            {
                if (!CharacterBrain.characterActions.crouch.value)
        ...
        protected virtual void ProcessPlanarMovement(float dt)
        {
            ...
            switch (CharacterActor.CurrentState)
            {
                ...
                case CharacterActorState.StableGrounded:
                    ...
                    {
                        if (CharacterBrain.characterActions.run.Started)
                            wantToRun = !wantToRun;
                    }
                    else
                    {
                        wantToRun = CharacterBrain.characterActions.run.value;
                    }
        ...
        JumpResult CanJump()
        {
            ...
            switch (CharacterActor.CurrentState)
            {
                case CharacterActorState.StableGrounded:
                    ...
                    if (CharacterBrain.characterActions.jump.StartedElapsedTime <= verticalMovementParameters.preGroundedJumpTime && groundedJumpAvailable)
                        jumpResult = JumpResult.Grounded;

                    break;
                case CharacterActorState.NotGrounded:

                    if (CharacterBrain.characterActions.jump.Started)
                    {
                ...
                case CharacterActorState.UnstableGrounded:
                    ...
                    if (CharacterBrain.characterActions.jump.StartedElapsedTime <= verticalMovementParameters.preGroundedJumpTime && verticalMovementParameters.canJumpOnUnstableGround)
                        jumpResult = JumpResult.Grounded;
        ...
        public void ProcessJump(float dt)
        {
        ...
        {
            return isCrouched && CharacterBrain.characterActions.jump.Started;
        }
        ...
        public void ProcessRegularJump(float dt)
        {
            if (CharacterActor.IsGrounded)
            {
                notGroundedJumpsLeft = verticalMovementParameters.availableNotGroundedJumps;
            ...
            if (isAllowedToCancelJump)
            {
                ...
                {
                    if (CharacterBrain.characterActions.jump.StartedElapsedTime >= verticalMovementParameters.cancelJumpMaxTime || CharacterActor.IsFalling)
                    {
                        isAllowedToCancelJump = false;
                    }
                    else if (!CharacterBrain.characterActions.jump.value && CharacterBrain.characterActions.jump.StartedElapsedTime >= verticalMovementParameters.cancelJumpMinTime)
                    {
            ...
            else
            {
                ...
                if (CharacterActor.IsGrounded)
                {
                    ...
                    if (OnGroundedJumpPerformed != null)
                    {
                        ...
                        Debug.Log("OnGroundedJumpPerformed");
                    }
                }
                else
                {
                    if (OnNotGroundedJumpPerformed != null)
                    {
                        ...
                        Debug.Log(" OnNotGroundedJumpPerformed(notGroundedJumpsLeft);");
                    }
        ...
        public override void PreCharacterSimulation(float dt)
        {
            ...
            CharacterStateController.Animator.SetFloat(horizontalAxisParameter, CharacterBrain.characterActions.movement.value.x);
            CharacterStateController.Animator.SetFloat(verticalAxisParameter, CharacterBrain.characterActions.movement.value.y);
        ...
        protected virtual void HandleSize(float dt)
        {
            ...
            if (crouchParameters.enableCrouch)
            {
                ...
                {
                    if (CharacterBrain.characterActions.crouch.Started)
                        wantToCrouch = !wantToCrouch;
                }
                else
                {
                    wantToCrouch = CharacterBrain.characterActions.crouch.value;
                }
