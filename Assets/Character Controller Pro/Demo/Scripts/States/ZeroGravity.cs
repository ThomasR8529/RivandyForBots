...
namespace Lightbug.CharacterControllerPro.Demo
{
    public class ZeroGravity : CharacterState
    {
        ...
        {
            pitchValue = Mathf.Lerp(pitchValue, pitchModifier * CharacterBrain.characterActions.pitch.value * pitchAngularSpeed * dt, pitchLerpAcceleration * dt);
            rollValue = Mathf.Lerp(rollValue, rollModifier * CharacterBrain.characterActions.roll.value * rollAngularSpeed * dt, rollLerpAcceleration * dt);
        ...
        private void ProcessVelocity(float dt)
        {
            ...
            CharacterActor.Velocity = Vector3.MoveTowards(CharacterActor.Velocity, targetVelocity, (CharacterBrain.characterActions.movement.Detected ? acceleration : deceleration) * dt);

            if (CharacterBrain.characterActions.jump.value)
            {
            ...
            else if (CharacterBrain.characterActions.crouch.value)
            {
                targetVerticalVelocity = -CharacterActor.Up * baseSpeed;
                CharacterActor.VerticalVelocity = Vector3.MoveTowards(CharacterActor.VerticalVelocity, targetVerticalVelocity, acceleration * dt);
            }
