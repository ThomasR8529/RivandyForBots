...
namespace PhysicsBasedCharacterController
{
    ...
    public class CharacterManager : MonoBehaviour
    {
        ...
        private struct JumpVelocityFactor
        {
            public float factor;
            public float duration;
            public float startTime;
        }
        private readonly List<JumpVelocityFactor> jumpVelocityFactors = new List<JumpVelocityFactor>();
        ...
        public void AddJumpVelocityFactor(float factor, float duration)
        {
            jumpVelocityFactors.Add(new JumpVelocityFactor { factor = factor, duration = duration, startTime = Time.time });
        }

        private void UpdateJumpVelocity()
        {
            float multiplier = 1f;
            for (int i = jumpVelocityFactors.Count - 1; i >= 0; i--)
            {
                var f = jumpVelocityFactors[i];
                if (Time.time > f.startTime + f.duration)
                {
                    jumpVelocityFactors.RemoveAt(i);
                }
                else
                {
                    multiplier *= f.factor;
